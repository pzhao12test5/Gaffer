...
public abstract class AbstractCoreKeyAccumuloElementConverter implements AccumuloElementConverter {
    ...
    protected abstract Pair<Boolean, MatchedVertex> getSourceAndDestinationFromRowKey(final byte[] rowKey,
    ...
    protected Edge getEdgeFromKey(final Key key, final Map<String, String> options) {
        ...
        final Pair<Boolean, MatchedVertex> directedMatchedVertex = getSourceAndDestinationFromRowKey(key.getRowData().getBackingArray(), result, options);
        String group;
        try {
        ...
        try {
            final Edge edge = new Edge(group, schema.getVertexSerialiser().deserialise(result[0]),
                    schema.getVertexSerialiser().deserialise(result[1]), directedMatchedVertex.getFirst(), directedMatchedVertex.getSecond());
