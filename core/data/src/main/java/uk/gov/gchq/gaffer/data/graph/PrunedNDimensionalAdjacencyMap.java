...
public class PrunedNDimensionalAdjacencyMap<T, U> {
    ...
    public void add(final AdjacencyMap<T, U> adjacencyMap) {
        if (!adjacencyMaps.isEmpty()) {
            ...
            for (final T source : adjacencyMap.getAllSources()) {
                if (!prevDestinations.contains(source)) {
                    prev.removeAllWithDestination(source);
                }
            }
        }
    ...
    public AdjacencyMap<T, U> get(final int n) {
        return adjacencyMaps.get(n);
    }
}
